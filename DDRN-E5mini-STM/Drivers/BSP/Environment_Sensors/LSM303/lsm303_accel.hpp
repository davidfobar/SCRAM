...
public:
  ...
  lsm303AccelData getAccelData(void);

private:
  I2C_HandleTypeDef *pntr_hi2c;
  uint8_t lsm303_I2C_addr;

  lsm303AccelData data; // Last read accelerometer data will be available here
  float getLSB(lsm303_accel_mode_t);
  uint8_t getShift(lsm303_accel_mode_t);

  bool write(const uint8_t reg, const uint8_t *data, const uint8_t len);
  bool read(const uint8_t reg, uint8_t *data, const uint8_t len);
};

#endif //LSM303_ACCEL_H
